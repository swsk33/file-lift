...
<span id="5.3">对于</span>集群或者分布式场景，使用S3对象存储作为文件储存的方式也是非常好的。
...
最新版本可在Maven中央仓库检查：[传送门](https://central.sonatype.com/artifact/software.amazon.awssdk/bom)
...
然后配置你的S3文件系统的服务器地址、密钥和存放文件的储存桶名称即可：

```properties
io.github.swsk33.file-lift.s3.endpoint=S3 地址
io.github.swsk33.file-lift.s3.access-key=S3 Access Key
io.github.swsk33.file-lift.s3.secret-key=S3 Secret Key
io.github.swsk33.file-lift.s3.bucket-name=存放文件的桶名称
```

如果配置的存放文件的桶不存在，则会在初次访问S3文件系统时自动创建。

## 6，手动创建`UploadFileService`实例

对于非Spring Boot环境，或者说需要多套存储配置情况下，我们还可以手动创建配置，主要流程：

- 创建`CoreConfig`实例
- 创建对应的存储方案配置实例
- 用两个配置实例创建`UploadFileServiceImpl`实例

### (1) 引入依赖

首先引入`file-lift-core`依赖：
...
如果要使用GirdFS存储文件，则需额外引入`mongodb-driver-sync`依赖，最新版可在[Maven中央仓库](https://central.sonatype.com/artifact/org.mongodb/mongodb-driver-sync)检查：
...
如果要使用S3文件系统存储文件，则需额外引入`aws-sdk`相关依赖，参考上述[基于S3协议文件系统的储存方式](#5.3)这一章节。
