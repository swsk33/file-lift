...
当使用MongoDB储存文件时，只需正确地配置Spring Boot的MongoDB数据源即可，需要注意Spring Boot 3.x和4.x版本的MongoDB配置存在差异：
    ...
    swsk33:
      file-lift:
        ...
        s3:
          ...
          # 默认：null
          secret-key: "your sk"
          # 地区，对于自部署的S3服务通常不需要配置该项
          # 默认：us-east-1
          region: "us-east-1"
          # 强制使用路径风格，一般自部署或国内厂商S3服务通常要开启
          # 否则，使用虚拟主机 (Virtual Hosted) 风格
          # 默认：true
          path-style: true
          # 存放文件的储存桶名称
