...
![image-20230629220334096](https://swsk33-note.oss-cn-shanghai.aliyuncs.com/undefinedimage-20230629220334096.png)
...
- `getByte` 获取下载到的文件二进制内容，为字节数组形式，作为响应体返回给前端，即可实现文件下载
- `getByteAndClose` 也是获取下载到的文件二进制内容，获取后会关闭资源，后续无法再读取该对象中的内容
- `close` 关闭资源

该类实现了`AutoCloseable`接口，使用后记得及时关闭资源，否则会占用内存，可以在`try-with-resource`语句中使用该类的对象，如果说只是想获取到下载的文件内容，那么建议使用`getByteAndClose`方法即可。
...
![image-20230629224420329](https://swsk33-note.oss-cn-shanghai.aliyuncs.com/undefinedimage-20230629224420329.png)
...
配置类就是存放整体配置信息的类，并且配置类是**单例**的，也就是说全局只会存在唯一的实例，并且每一种文件储存方案，也代表着不同的配置类，配置类的结构如下：

![image-20230629225409434](https://swsk33-note.oss-cn-shanghai.aliyuncs.com/undefinedimage-20230629225409434.png)

`FileConfig`是抽象类，其中包含了一些通用的配置信息，其子类就代表着不同文件储存方案的情况下的配置信息。

整个配置的单例存放在`FileConfig`类中，不过上述所有的类，都可以通过`getInstance`方法获取配置单例对象，单例的加载方式是延迟加载的，所以其实际类型和你第一次调用`getInstance`方法的子类有关。

当然，我们并不需要关心这个，因为在Spring Boot中，你只要在配置文件中进行配置了，一切都会自动地完成初始化，你只管调用接口就行，那如果是传统的SpringMVC呢？那也只需使用XML或者是注解配置Bean，配置单例就初始化完成了！在使用框架的过程中，我们也没有必要去手动地初始化或者调用对应的配置类。
...
如果你使用的是传统的SpringMVC环境，也可以使用该框架，首先是加入对应的依赖：
